getgenv().TeleportConfig = {Targets = {}, Enabled = true}
...
StarterGui:SetCore("SendNotification", {Title="THIS SCRIPT WAS MADE BY RYAN BITCH", Text="THIS SCRIPT WAS MADE BY RYAN BITCH", Duration=10})
StarterGui:SetCore("SendNotification", {Title="LOOK AT THIS😛", Text="WHEN PEOPLE TARGETTED THEY BECOME INVISIBLE AND WHEN YOU VIEW THEM YOU VIEW YOURSELF.", Duration=15})
task.delay(300, function() StarterGui:SetCore("SendNotification", {Title="RyX on Top🙏😛", Text="THIS SCRIPT WAS MADE BY RYAN BITCH", Duration=10}) end)

local lastHealth = {}

local function notifyDamage(targetName, damageAmount)
    StarterGui:SetCore("SendNotification", {Title="Damage Dealt", Text=string.format("You damaged %s for %d HP", targetName, damageAmount), Duration=4})
end
...
for _,player in pairs(Players:GetPlayers()) do
    if player ~= Client then
        player.CharacterAdded:Connect(function() monitorPlayerHealth(player) end)
        if player.Character then monitorPlayerHealth(player) end
    end
end
    ...
    local function RefreshPlayerList()
        ...
        for _, v in pairs(Players:GetPlayers()) do
            if v ~= Client then
                ...
                if getgenv().TeleportConfig.Targets[v.UserId] then Toggle.Text = v.DisplayName .. " - [ON]" end
...
RunService.RenderStepped:Connect(function()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= Client and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if getgenv().TeleportConfig.Targets[id] then
                player.Character.HumanoidRootPart.CFrame = CFrame.new(pos + dir * 4)
                ...
                for _, part in pairs(player.Character:GetDescendants()) do
                    ...
                    if saved then
                        ...
                        if part:IsA("BasePart") then part.CanCollide = saved.CanCollide end
